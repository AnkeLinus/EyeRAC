...
class TaskDetection(Node):
    def __init__(
        ...
        self.bounding_box_publisher = self.create_publisher(
            BoundingBoxArray, 
            "/task_detection_bounding_box", 
            1
        )

    def et_camera_callback(self, image: Image) -> None:
        if image is None:
        ...
        if self.bounding_box_array_msg.bounding_boxes == []:
            self._basic_logger.debug("Empty Bounding Boxes. Not publishing.")
        else:
